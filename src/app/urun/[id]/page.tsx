...
export default function UrunDetayPage() {
  ...
  const handleShare = () => {
    const priceText = product.priceOnRequest ? 'Fiyat için sorun' : formatPrice(product.price);
    const text = `${formatProductName(product.name)} - ${priceText} | Favian Jewellery`;
  ...
  const handleAskSeller = () => {
    const text = `Merhaba, "${formatProductName(product.name)}" (Kod: ${product.code}) ürününün fiyatı ve stok durumu hakkında bilgi alabilir miyim?`;
    const whatsappUrl = `https://wa.me/${whatsappNumber}?text=${encodeURIComponent(text)}`;
    window.open(whatsappUrl, '_blank');
  };
    ...
    <div className="min-h-screen bg-white">
      <div className="container mx-auto px-4 py-8">
        ...
        <nav className="flex flex-wrap items-center gap-2 text-sm mb-6">
          ...
          <span className="text-stone-400">/</span>
          <span className="font-serif font-semibold text-stone-900 truncate max-w-[200px]">{formatProductName(product.name)}</span>
        </nav>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 lg:gap-12">
          ...
          <div className="min-w-0">
            ...
            {/* Urun Adi */}
            <h1 className="font-serif text-2xl sm:text-3xl lg:text-4xl font-semibold text-stone-900 mb-4">{formatProductName(product.name)}</h1>
            ...
            <div className="flex flex-wrap items-center gap-2 sm:gap-4 mb-6">
              {product.priceOnRequest ? (
                <span className="font-serif text-2xl sm:text-3xl font-semibold text-stone-900 tracking-normal">
                  Satıcıya Sor
                </span>
              ) : (
                <>
            ...
            {/* Miktar Secimi — sadece fiyat varsa */}
            {!product.priceOnRequest && (
            ...
            {/* Sepete Ekle Butonu — sadece fiyat varsa göster */}
            {!product.priceOnRequest && (
              <>
                <Button
                  size="lg"
        ...
        <div className="mt-12 border-t border-stone-200 pt-8">
          <h2 className="font-serif text-2xl sm:text-3xl font-semibold text-stone-900 mb-4">Mücevher Hakkında</h2>
